...
window.addEventListener("resize", () => {
    width = window.innerWidth - 800 - margin.left - margin.right;
    svg.attr("width", width + margin.left + margin.right);
    updateBarChartData(nodeToSummary); // Atualize os dados para refletir a nova largura
});
...
if (global_selected_mode === "usuário") {
    d3.select("#applet-title").text("participação dos usuários");
...
function updateBarChartData(newData) {
    ...
    newData = newData.filter(d => d.soma > 0);

    // Ordenar os dados pela coluna "soma"
    newData = newData.sort((a, b) => b.soma - a.soma);
    ...
    function fontSize() {
        return Math.min(12, y.bandwidth() / 3) + "px";
    }

    const bars = g.selectAll(".bar").data(newData, d => d.idPessoa);
    const barLabels = g.selectAll(".bar-label").data(newData, d => d.idPessoa);
    ...
    barLabels.enter().append("text")
        ...
        .attr("y", d => y(d.name) + y.bandwidth() / 2)  // centraliza o texto na barra
        .attr("x", d => x(d.soma) - 5)  // posiciona o texto um pouco à esquerda do final da barra
        .attr("text-anchor", "end")  // alinha o texto à direita
        .attr("alignment-baseline", "middle")  // centraliza verticalmente
        .attr("fill", "white")  // define a cor do texto como branca
        .style("font-size", d => fontSize)  // Aplica o tamanho da fonte
        .text(d => d.soma)
        .merge(barLabels)
        .transition().duration(200)
        .attr("y", d => y(d.name) + y.bandwidth() / 2)
        .attr("x", d => x(d.soma) - 5)
        .style("font-size", d => fontSize)  // Aplica o tamanho da fonte na atualização também
        .text(d => d.soma);
    ...
    g.select(".y.axis")
        .transition().duration(1000) // Adicione a transição para uma atualização suave
        .call(d3.axisLeft(y))
        .style("font-size", () => Math.max(16, y.bandwidth() / 1.5) + "px");
}
